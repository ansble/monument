...
const test = require('ava'),
  ...
  return mock;
      ...
      withPath = filePath => {
  return path.join(process.cwd(), 'test_stubs/deletes/static/', filePath);
};
...
test.cb('should handle a static file', t => {
  ...
  handleStaticFile(withPath('main.js'), connectionMock, {
    maxAge: 1000,
    compress: false
  });
});
...
test.cb('should handle the HEAD to a static file', t => {
  ...
  handleStaticFile(withPath('main.js'), connectionMock, { maxAge: 1000, compress: false });
});
