...
test('should remove the whole route if all verbs are passed to it', t => {
  ...
  t.is(typeof routeStore.remove('/test', ['get', 'put', 'post']).standard['/test'], 'undefined');

  t.not(typeof routeStore.get().standard['/hobbits'], 'undefined');

  t.is(typeof routeStore.remove('/hobbits', 'get').standard['/hobbits'], 'undefined');

  t.is(typeof routeStore.remove('/hobbits/:name', 'get').wildcard['/hobbits/:name'], 'undefined');

  t.is(typeof routeStore.remove('/rangers/:name', allVerbs).wildcard['/rangers/:name'], 'undefined');
});
