...
The main focus of this release is expanding the available protocol stack that `monument` can use to communicate. It adds a built-in Web Socket server and the ability to drop in any http-like server that adheres to the API made popular by the built-in `http` and `https` modules. This includes `spdy` and `http2`!
